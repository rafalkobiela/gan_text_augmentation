...
def init_dict(dataset):
    ...
    """
    tokens = get_tokenlized('data/{}.txt'.format(dataset))
    word_set = get_word_list(tokens)
    word2idx_dict, idx2word_dict = get_dict(word_set)

    with open('data/{}_wi_dict.txt'.format(dataset), 'w') as dictout:
        dictout.write(str(word2idx_dict))
    with open('data/{}_iw_dict.txt'.format(dataset), 'w') as dictout:
...
def load_dict(dataset):
    ...
    iw_path = 'data/{}_iw_dict.txt'.format(dataset)
    wi_path = 'data/{}_wi_dict.txt'.format(dataset)
...
def load_test_dict(dataset):
    ...
    tokens = get_tokenlized('data/testdata/{}_test.txt'.format(dataset))
    word_set = get_word_list(tokens)
    ...
    for sent in tensor:
        ...
        for word in sent.tolist():
            if word == cfg.padding_idx:
                break
            sent_token.append(dictionary.get(str(word), str(0)))
